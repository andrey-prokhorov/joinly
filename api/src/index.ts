...
// Only create swagger spec if enabled (dev only)
...
async function initializeSwagger() {
	if (config.swagger.enabled) {
		try {
			...
			logger.warn(
				"Failed to load swagger-jsdoc, swagger docs will be disabled",
			);
		}
	}
}

// Initialize swagger asynchronously
initializeSwagger();
...
if (config.swagger.enabled && openApiSpec) {
